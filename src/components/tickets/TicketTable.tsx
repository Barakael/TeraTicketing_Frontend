...
const TicketTable: React.FC<TicketTableProps> = ({
  ...
  const formatUser = (user: any): string => {
    ...
    if (user.name) {
      return user.name === "Unknown" || user.name === "Guest"
        ? "Guest"
        : user.name;
    }
    return "Guest";
              ...
              <td className="px-6 py-4">
                <Badge variant={getPriorityColor(ticket.priority)}>
                  {typeof ticket.priority === "string"
                    ? ticket.priority
                    : (ticket.priority as any)?.name || "N/A"}
                </Badge>
              </td>
              <td className="px-6 py-4">
                <Badge variant={getStatusColor(ticket.status)}>
                  {(typeof ticket.status === "string"
                    ? ticket.status
                    : (ticket.status as any)?.name || "N/A"
                  )?.replace("_", " ")}
                </Badge>
              </td>
              <td className="px-6 py-4">{formatField(ticket.department)}</td>
              <td className="px-6 py-4">{formatField(ticket.category)}</td>
              <td className="px-6 py-4">{formatUser(ticket.createdBy)}</td>
              <td className="px-6 py-4">{formatUser(ticket.assignedTo)}</td>
              <td className="px-6 py-4">
